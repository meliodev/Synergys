...
import { faCheckCircle, faCheck, faFlag, faTimes, faClock, faUpload, faFileSignature, faSackDollar, faEnvelopeOpenDollar, faEye, faPen, faBan, faSpinner } from '@fortawesome/pro-light-svg-icons'
...
const ProcessAction = ({ process, processMain, ProjectId, navigation, ...props }) => {
    ...
    useEffect(() => {
        const { choices } = currentAction
        if (choices) {
            var elementsTemp = choices.map((choice) => configChoiceIcon(choice))
        ...
        const onSelectChoice = async (choice) => {
            const { commentRequired, onSelectType, operation } = choice
            ...
            if (commentRequired) {
                ...
                if (onSelectType === 'navigation') {
                    ...
                    await validateAction(null, nextStep, nextPhase)
                }

                else if (onSelectType === 'validation') {
                    await runChoiceOperation(operation)
                    await validateAction(null, null, null)
                }

                else if (onSelectType === 'commentPicker') {
                    if (choice.subChoices) {
                        var elementsTemp = choice.subChoices.map((choice) => configChoiceIcon(choice))
                        setElements(elementsTemp)
                    }
                        ...
                        // await runChoiceOperation(operation)
                        // await validateAction(choice.label, null, null)
                    }
                }
